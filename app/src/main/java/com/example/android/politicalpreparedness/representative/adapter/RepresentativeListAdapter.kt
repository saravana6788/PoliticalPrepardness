...
class RepresentativeListAdapter: ListAdapter<Representative, RepresentativeViewHolder>(
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RepresentativeViewHolder {
        val binding = RepresentativeDetailBinding.inflate(LayoutInflater.from(parent.context))
...
class RepresentativeViewHolder(val binding: RepresentativeDetailBinding): RecyclerView.ViewHolder(binding.root) {
    ...
    private fun showSocialLinks(channels: List<Channel>) {
        ...
        if (!facebookUrl.isNullOrBlank()) { enableLink(binding.facebookImage, facebookUrl) }

        val twitterUrl = getTwitterUrl(channels)
        if (!twitterUrl.isNullOrBlank()) { enableLink(binding.twitterImage, twitterUrl) }
    }

    private fun showWWWLinks(urls: List<String>) {
        enableLink(binding.wwwImage, urls.first())
    }
...
val representativeDiffCallback = object:DiffUtil.ItemCallback<Representative>(){
